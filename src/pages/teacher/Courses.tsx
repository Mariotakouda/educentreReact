...
const TeacherCourses: React.FC = () => {
  ...
  const fetchCourses = async () => {
    try {
      const response = await apiService.teacherGetCourses();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingCourse) {
        await apiService.teacherUpdateCourse(editingCourse.id, formData);
        toast.success('Formation mise à jour avec succès');
      } else {
        await apiService.teacherCreateCourse(formData);
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      await apiService.teacherDeleteCourse(id);
        ...
        {courses.length === 0 ? (
          <Card>
            <p className="text-center text-gray-500 py-8">
              Vous n'avez pas encore créé de formation. Commencez maintenant !
            </p>
          </Card>
        ) : (
