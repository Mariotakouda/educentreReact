...
const TeacherModules: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingModule) {
        await apiService.teacherUpdateModule(editingModule.id, formData);
        toast.success('Module mis à jour avec succès');
      } else {
        await apiService.teacherCreateModule(formData);
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      await apiService.teacherDeleteModule(id);
    ...
    <div className="min-h-screen bg-gray-50 py-8">
      <div className="container mx-auto px-4">
        <div className="flex justify-between items-center mb-8">
          <h1 className="text-3xl font-bold text-gray-800">Mes Modules</h1>
        ...
        {courses.length === 0 ? (
          ...
          <Card>
            <p className="text-center text-gray-500 py-8">
              Vous n'avez pas encore créé de module. Commencez maintenant !
            </p>
          </Card>
        ) : (
